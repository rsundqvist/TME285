...
{
    public partial class SR1 : Form
    {
        private IsolatedWordRecognizer recognizer = null;
        private WAVSound testSound = null;
        private RecognitionResult recognitionResult; // The result of the most recent call to IsolatedWordRecognizer.RecognizeSingle()
        ...
        private void NumberChanged(object sender, EventArgs e)
        {
            try
            {
                int newWordNumber = int.Parse(numberBox.Text);
                if (newWordNumber >= 0)
                    currentWordNumber = newWordNumber;
            }
            catch (Exception)
            {
                //Whatever.
        ...
        private void quickSave_Click(object sender, EventArgs e)
        {
            ...
            if (folder == null) ShowQuickSaveFolderDialog();

            string filename = word + "_" + currentWordNumber + ".wav";
            WAVSound savedSound = soundVisualizer.Sound.Copy();
            savedSound.SaveToFile(folder + "\\" + filename);

            numberBox.Text = (currentWordNumber + 1) + "";
        }

        private string folder = null;
        private void ShowQuickSaveFolderDialog()
        {
        ...
        private void ChangeQSFolder(object sender, EventArgs e)
        {
            ShowQuickSaveFolderDialog();
        }

        private void trainRecognizer(object sender, EventArgs e)
        {
            ...
            selectFolderDialog.Description = "Select root folder.";
            string trainingRootFolder;
            if (selectFolderDialog.ShowDialog() == DialogResult.OK)
            {
                trainingRootFolder = selectFolderDialog.SelectedPath;
                ea.importTestSounds(trainingRootFolder);
                ea.importUnknownSounds(trainingRootFolder + "\\_unknown");
            }
